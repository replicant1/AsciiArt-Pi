...
| 15 | `answer.put_nowait(reply text)` | [Every line is answered](../../ascii_camera.py#L470), including the ones that changed nothing, so no client is ever left waiting on a reply that is not coming |
...
| 17 | `contrast 1.0->2.4, invert False->True` | What the person sees, in the same wording a keypress would have put in the [status line](../../ascii_camera.py#L716) |
